...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("LEDRP Status Bot", "https://media.discordapp.net/attachments/792809232260923453/797775071451676682/96x96.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ LEDRP' : '⚫ LEDRP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nJak wejsc na serwer','Klikasz F8 --> wpisujesz connect 62.171.189.1:45632',false)
    if (STATUS !== undefined)
    {
      embed.addField(':warning: Actuele Status serwera:',`${STATUS}\n\u200b\n`);
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        .addField('Gracze:',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
  ...
  bot.on('ready',() => {
    ...
    // bot.user.setGame('Roofstad', 'LEDRP');
    bot.user.setActivity('LEDRP',{'url':'LEDRP','type':'STREAMING'});
